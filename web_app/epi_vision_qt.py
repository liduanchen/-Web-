...
class EpiVisionQtWindow(QMainWindow):
    def __init__(self):
        ...
        hdr5 = QLabel("n范围\n~峰距")
        hdr6 = QLabel("参考\n折射率")
        for h in (hdr0, hdr1, hdr2, hdr3, hdr4, hdr5, hdr6):
            h.setWordWrap(True)
        ...
        pg.addWidget(sr, r, 5)
        pg.addWidget(self.combo_preset_n, r, 6)
        ...
        # 信号
        self.combo_preset_n = QComboBox()
        self.combo_preset_n.addItems(["折射率参考：不填入", "SiC", "Si", "GaN", "GaAs"])
        self.combo_preset_n.currentTextChanged.connect(self.on_preset_n_changed)
    ...
    def on_preset_n_changed(self, text: str):
        """仅当用户在下拉框中选择材料名时，将参考折射率填入输入框（切换数据集不再自动覆盖）。"""
        if not text or "不填入" in text:
            return
        preset = {
            "SiC": (2.60, 2.55),
            "Si": (3.40, 3.55),
